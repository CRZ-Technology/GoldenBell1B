 ...
 */
#define TEMP_SENSOR_ONCHIP_ADDRESS  0x40 
    ...
    while (1)
    {
        ...
        *TemperaturePtr = 0xE3;
        ...
        ByteCount = XIic_Recv(IicBaseAddress, TempSensorAddress,
                    TemperaturePtr, 3, XIIC_STOP);

        temperature = ((((((*TemperaturePtr) << 8) + (*(TemperaturePtr + 1))) * 17572) >> 16) - 4865) / 100;

		XUartLite_SendByte(UARTLITE_BASEADDR, temperature / 10 + '0');
		XUartLite_SendByte(UARTLITE_BASEADDR, temperature % 10 + '0');
		XUartLite_SendByte(UARTLITE_BASEADDR, '\r');
